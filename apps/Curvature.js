const Curvature = {
    ...
    run: async (process) => {
        ...
        const Icon = (icon: string) => new HTML('i').class('material-symbols-rounded').text(icon).style({
            opacity: '0.6'
        })
        const Flex = () => new HTML('div').style({
            flex: '1'
        })
        const Divider = () => new HTML('div').styleJs({
            width: '100%',
            borderBottom: '1px solid rgba(0, 0, 0, 0.1)'
        })
        const Tab = (icon: string, title: string, active: boolean) => {
        ...
        const sites: Array < {
            ...
            },
            {
                ...
                icon: 'https://s2.googleusercontent.com/s2/favicons?domain=minecraft.net'
            },
            {
                name: 'Google',
...
`)
        ...
        const actions = new HTML('div').appendTo(sidebar).styleJs({
            display: 'flex',
            gap: '10px'
        }).appendMany(
        ...
        sidebar.append(Tab('add', 'New Tab', false).on('click', () => {
            tabsArray.push(new BrowserTab('https://www.google.com/webhp?igu=1'))
        }))

        for (const site of sites) {
            new HTML('button').appendTo(buttons).styleJs({
                ...transparentStyles,
                flex: '1',
                display: 'flex',
                aspectRatio: '1 / 1',
        ...
        const genTabID = () => Math.random().toString(36).substring(7)

        class BrowserTab {
            tab: HTML | undefined
            tabElement: HTML | undefined
            ...
            render = () => {
                this.tab = Tab(`https://t3.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=${this.url}&size=64`, 'Tab', false)
                ...
                this.tabElement.on('load', () => {
                    ...
                    this.tab?.qs('span')?.text(this.tabElement?.elm.contentDocument?.title || 'Tab')
